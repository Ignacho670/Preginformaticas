...
let score = 0;
const initialTimeLimit = 35; // Tiempo inicial del juego en segundos
...
function startGameTimer() {
    const clock = document.getElementById("animated-clock");
    const clockHand = document.querySelector(".clock-hand");  // La manecilla del reloj
    ...
    timerInterval = setInterval(() => {
        ...
        // Cambiar color de la manecilla dependiendo del tiempo restante
        if (timeLeft > 20) {
            clockHand.classList.remove("yellow", "red");
            clockHand.classList.add("green");
        } else if (timeLeft > 10) {
            clockHand.classList.remove("green", "red");
            clockHand.classList.add("yellow");
        } else if (timeLeft > 0) {
            clockHand.classList.remove("green", "yellow");
            clockHand.classList.add("red");
        }
    ...
    }, 1000); // El intervalo se mantiene a 1000 ms
}
...
function checkAnswer(selectedButton, selectedOption) {
    ...
    if (selectedOption === correctAnswer) {
        ...
        score++;
        timeLeft += 10; // Incrementar el tiempo en 10 segundos si la respuesta es correcta
    } else {
...
document.getElementById("start-button").addEventListener("click", () => {
    ...
    shuffle(questions);  // Mezclar las preguntas al inicio del juego
    startGameTimer();
    loadQuestion();
});
